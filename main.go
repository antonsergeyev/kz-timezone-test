...
func main() {
	...
	fmt.Printf(
		"Текущее смещение: %s (%d секунд), действует от %s до %s\n",
		offsetName, offset,
	...
	if marchOffset != 60*60*5 {
		isOk = false
		fmt.Printf("👎 Смещение в марте не равно ожидаемому. Вероятно, пакет tzdata не обновлён, либо не установлен, либо не настроена текущая таймзона.\n")
	} else {
		fmt.Printf("👍 Смещение в марте равно ожидаемому. Вероятно, пакет tzdata уже обновлён.\n")
	}
...
func checkPhpTimezone(phpPath string) (string, error) {
	...
	if string(out) != expected {
		return "", fmt.Errorf(
			`👎 Смещение в марте на PHP %s не равно ожидаемому: получили %s, ожидалось %s секунд. `+
		...
		return fmt.Sprintf("👍 Смещение в марте на PHP %s равно ожидаемому: %s", phpPath, string(out)), nil
	}
}
